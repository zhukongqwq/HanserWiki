...
public class LLMClient
{
    ...
    /// <summary>
    /// 流式调用对话接口：POST stream:true，逐块解析 SSE 的 delta.content 并回调 onDelta。
    /// dry_run 时模拟逐块流式输出（每块约 3-4 字、40ms 间隔）。
    /// </summary>
    public async Task StreamChatAsync(List<ChatMessage> messages, Action<string> onDelta,
        double temperature = 0.3, int maxTokens = 255000, double topP = 0.9)
    {
        if (_dryRun)
        {
            var full = MockChat(messages);
            for (var i = 0; i < full.Length;)
            {
                var len = Math.Min(3 + (i % 2), full.Length - i);
                onDelta(full.Substring(i, len));
                i += len;
                await Task.Delay(40);
            }
            return;
